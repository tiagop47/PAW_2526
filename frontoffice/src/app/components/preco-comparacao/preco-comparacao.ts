...
@Component({
  selector: 'app-preco-comparacao',
  imports: [CommonModule],
  templateUrl: './preco-comparacao.html',
  styleUrl: './preco-comparacao.css',
})
export class PrecoComparacao implements OnChanges {
  @Input() produtoBase: ProductDTO | null = null;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['produtoBase'] && this.produtoBase) {
      this.carregarComparacao(this.produtoBase);
    }
  }

  private carregarComparacao(produto: ProductDTO): void {
    this.loading = true;

    this.productService.compararPorNome(produto.nome).subscribe({
      next: (resultados) => {
