...
adminService.eliminarCategoria = async function (id) {
  ...
  if (produtosUsam > 0) {
    ...
    if (!categoriaOutros) {
      categoriaOutros = await Category.create({
        nome: "Outros",
    ...
    await Product.updateMany(
      { categoriaId: id },
      { categoriaId: categoriaOutros._id },
    );
  }
...
adminService.getPendentesDocumentos = async function (
  pagina,
  limite,
  filtroTipo = null,
) {
  ...
  if (!filtroTipo || filtroTipo === "supermercados") {
    const [total, items] = await Promise.all([
      ...
      Supermarket.find(querySupermarket)
        .populate("userId")
        .sort({ criadoEm: -1 })
        .lean(),
    ]);
  ...
  resultados.sort(
    (a, b) => new Date(b.dataPendente) - new Date(a.dataPendente),
  );
...
adminService.getUsersDocumentos = async function (
  pagina,
  limite,
  filtroRole = null,
...
adminService.atualizarUserById = async function (id, dados) {
  ...
  if (
    roleAntigo === "supermercados" &&
    userAtualizado.role !== "supermercados"
  ) {
...
adminService.getUtilizadoresCandidatos = async function () {
  const usersComRole = await User.find({ role: "supermercados" })
  ...
  const idsSet = new Set(
    idsComSupermercado.filter((id) => id).map((id) => id.toString()),
  );
 ...
 */
adminService.transferirPropriedade = async function (
  supermercadoId,
  novoUserId,
) {
  ...
  return Supermarket.findByIdAndUpdate(
    supermercadoId,
    {
      userId: novoUserId,
      estadoAprovacao: "Pendente",
 ...
 */
adminService.getProdutosPaginados = async function (
  pagina,
  limite,
  supermercadoId = null,
) {
 ...
 */
adminService.obterDadosModeracaoProdutos = async function (
  pagina,
  limite,
  supermercadoId,
) {
  const dadosPagina = await adminService.getProdutosPaginados(
    pagina,
    limite,
    supermercadoId,
  );
...
adminService.getMercadosAtivos = async function (pagina, limite) {
  ...
  const total = await Supermarket.countDocuments({
    estadoAprovacao: { $in: ["Aprovado", "Bloqueado"] },
  });
 ...
 */
adminService.getEncomendasPaginadas = async function (
  pagina,
  limite,
  supermercadoId = null,
) {
...
adminService.getFaturaEncomenda = async function (orderId) {
  ...
  if (!encomenda.faturaNumero) {
    const erro = new Error(
      "Esta encomenda ainda não tem uma fatura gerada. A fatura é criada automaticamente quando a encomenda é confirmada ou entregue.",
    );
