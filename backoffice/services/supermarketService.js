...
supermarketService.obterDadosDashboard = async function (supermercadoId) {
    ...
    Order.aggregate([
      ...
      { $unwind: "$produtos" },
      {
        $group: {
          ...
          faturado: {
            $sum: {
              $multiply: ["$produtos.quantidade", "$produtos.precoUnitario"],
            },
          },
        },
      },
  ...
  const valorMedio =
    totalEncomendas > 0
      ? parseFloat((vendasTotais / totalEncomendas).toFixed(2))
      : 0;
  ...
  return {
    ...
    top5Produtos,
    mediaAvaliacao:
      avaliacaoStats.length > 0
        ? parseFloat(avaliacaoStats[0].media.toFixed(1))
        : null,
...
supermarketService.getAllSupermercados = async function () {
  return Supermarket.find({ estadoAprovacao: "Aprovado" }).select(
    "_id nome descricao localizacao localizacaoGeo raioEntregaKm custoEntregaPorMetodo",
  );
};
...
supermarketService.obterProdutos = async function (
  supermercadoId,
  pagina = 1,
  limite = 5,
) {
...
supermarketService.obterProdutoPorId = async function (
  id,
  supermercadoId = null,
) {
  const filtro = { _id: id };
...
supermarketService.criarProduto = async function (supermercadoId, productData) {
  ...
  if (!catalogProductId && nome) {
    let porNome = await CatalogProduct.findOne({
...
supermarketService.atualizarProduto = async function (
  supermercadoId,
  productId,
  updateData,
) {
...
supermarketService.listarProdutosGeral = async function (supermercadoId) {
  ...
  if (supermercadoId) query.supermercadoId = supermercadoId;
...
supermarketService.compararProdutosPorNome = async function (nome) {
  return Product.find({ nome: { $regex: nome, $options: "i" }, ativo: true })
    .populate("supermercadoId", "nome localizacao")
    .populate("categoriaId", "nome")
    .sort({ preco: 1 });
};

supermarketService.pesquisarProdutos = async function (
  supermercadoId,
  { q, categoriaId, pagina = 1, limite = 5 },
) {
  const filtro = { supermercadoId, ativo: true };
  if (q) {
    filtro.$or = [
...
supermarketService.atualizarSupermercado = async function (
  supermercadoId,
  dadosSupermercado,
) {
  const { latitude, longitude, custoEntregaPorMetodo, raioEntregaKm } =
    dadosSupermercado;
  ...
  if (custoEntregaPorMetodo) {
    dadosSupermercado.custoEntregaPorMetodo = {
      levantamento_loja:
        custoEntregaPorMetodo.levantamento_loja !== undefined
          ? parseFloat(custoEntregaPorMetodo.levantamento_loja) || 0
          : 0,
      entrega_domicilio:
        custoEntregaPorMetodo.entrega_domicilio !== undefined
          ? parseFloat(custoEntregaPorMetodo.entrega_domicilio) || 0
          : null,
    };
  }

  return Supermarket.findByIdAndUpdate(supermercadoId, dadosSupermercado, {
    new: true,
    runValidators: true,
  });
};
...
supermarketService.obterEncomendas = async function (
  supermercadoId,
  pagina = 1,
  limite = 5,
  filtroEstado = null,
...
supermarketService.obterEncomendaPorId = async function (
  supermercadoId,
  orderId,
) {
  return Order.findOne({ _id: orderId, supermercadoId }).populate(
    "clienteId",
    "nome email telefone",
  );
};
...
supermarketService.transicoesPermitidasParaEncomenda =
  transicoesPermitidasParaEncomenda;

supermarketService.atualizarEstadoEncomenda = async function (
  supermercadoId,
  orderId,
  estado,
  codigoFornecido = null,
) {
  ...
  const permitidos =
    transicoesPermitidasParaEncomenda(order)[estadoAnterior] || [];
  if (!permitidos.includes(estado)) {
    throw new Error(
      `Transição de estado inválida: ${estadoAnterior} → ${estado}`,
    );
  }

  // Verificação de Código de Levantamento (Segurança adicional)
  if (order.metodoEntrega === "levantamento_loja" && estado === "entregue") {
    if (!codigoFornecido || codigoFornecido !== order.codigoLevantamento) {
      throw new Error(
        "Código de levantamento inválido ou não fornecido. Peça o código ao cliente.",
      );
    }
  }
  ...
  try {
    ...
    io.to(`user_${order.clienteId}`).emit("status-encomenda", {
      encomendaId: order._id,
      novoEstado: estado,
      mensagem: `O estado da sua encomenda foi alterado para: ${estado}`,
    });
  } catch (socketErr) {
    console.error("Erro ao emitir socket de status:", socketErr);
  }
...
supermarketService.registarVenda = async function (supermercadoId, saleData) {
  ...
  return orderService.criarEncomenda(null, {
    ...
    coordenadasEntrega:
      latitudeEntrega && longitudeEntrega
        ? { lat: Number(latitudeEntrega), lng: Number(longitudeEntrega) }
        : undefined,
    origem: "caixa",
    clienteSnapshot: {
      nome: nomeCliente,
...
supermarketService.obterPromocoesHome = async function () {
  ...
  const lojas = await Product.aggregate([
    { $match: { precoAntigo: { $gt: 0 }, ativo: true } },
    {
      $group: {
        _id: "$supermercadoId",
